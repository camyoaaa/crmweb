<template>
    <a-card :bordered="false">
                ...
                <a-row :gutter="48">
                    <a-col :md="6" :sm="24">
                        <a-form-item label="收据编号">
                            <id-input v-model="queryParam.payreceiptid" placeholder="输入收据编号" />
                        </a-form-item>
                    </a-col>
                    <a-col :md="6" :sm="24">
                    ...
                    <a-col :md="6" :sm="24">
                        <a-form-item label="付款客户">
                            <id-input v-model="queryParam.customid" placeholder="输入客户编号" />
                        </a-form-item>
                    </a-col>
                    <a-col :md="6" :sm="24">
                    ...
                    <a-col :md="6" :sm="24">
                        <a-form-item label=" 收 款 人 ">
                            <staff-select v-model="queryParam.creator" role-name="销售员" placeholder="选择收款人" />
                        </a-form-item>
                    </a-col>
                    <a-col :md="6" :sm="24">
        ...
        defaultValue: [$moment('00:00:00', 'HH:mm'), $moment('23:59:59', 'HH:mm')],
                    ...
                    <a-col :md="6" :sm="24">
                        <a-form-item label="收据状态">
                            <a-select v-model="queryParam.status" allowClear placeholder="选择收据状态">
        ...
        onChange: onSelectChange
            ...
            <s-table size="middle" ref="table" rowKey="payreceiptid" :alert="false" :columns="columns" :data="loadData" showPagination="auto" :pagination="pagination">
                ...
                <span slot="reviewmark" slot-scope="text, record">
                    <a-input v-if="record.editing" v-model="record.reviewmark" :rows="1" size="small" />
                    <div v-else>
                        {{text || '无'}}
                    </div>
                </span>
                <span slot="reviewStatus" slot-scope="text, record">
                    <a-select v-if="record.editing" v-model="record.status" size="small">
                        <a-select-option v-for=" r in reviewStatusList" :value="r.id" :key="r.id" :disabled="editingCache.status === r.id">{{r.name}}</a-select-option>
                    </a-select>
                    <div v-else>
                ...
                <span slot="action" slot-scope="text,record">
                    <div v-if="record.editing">
                        <a-button size="small" icon="check" @click="reviewReceipt(record)" :disabled="record.status === editingCache.status"></a-button>
                        <a-button size="small" icon="close" @click="cancelTrans(record)"></a-button>
                    </div>
                    <div v-else>
                        <a-button v-if="record.status === 0" type="primary" size="small" @click="startReview(record)" :disabled="!$auth('receiptManage.review') || Object.keys(editingCache).length > 0">审核</a-button>
                        <a-button v-else size="small" @click="editReceipt(record)" :disabled="!$auth('receiptManage.modify') || Object.keys(editingCache).length > 0 || record.status === (reviewStatusList.find(r=>r.name=='审核通过').id)">更正</a-button>
                    </div>
...
export default {
    ...
    methods: {
        getStatusZn(status) {
            console.log("xxxxxxxxxxxxxxxxxxxxstatus", status);
