      ...
      >
        ...
        <span slot="cid" slot-scope="text, record">
          <router-link :to="{ path: '/customDetail', query: { cid: text } }"
            >{{ record.customName }}/{{ record.cid }}</router-link
          >
        </span>
        ...
        <span slot="reviewStatus" slot-scope="text, record">
          <a-select v-if="record.editing" v-model="record.status" size="small">
            <a-select-option
              ...
              :value="r.id"
              :key="r.id"
              :disabled="r.id === editingCache.status"
              >{{ r.name }}</a-select-option
            >
          </a-select>
          <div v-else>
        ...
        <span slot="action" slot-scope="text, record">
          <div v-if="record.editing">
            <a-button
              size="small"
              icon="check"
              @click="reviewContract(record)"
              :disabled="record.status === editingCache.status"
            ></a-button>
            <a-button
              size="small"
              icon="close"
              @click="cancelReview(record)"
            ></a-button>
          </div>
          <div v-else>
            <a-button
