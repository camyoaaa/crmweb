<template>
    <a-card :bordered="false">
                ...
                <a-row :gutter="48">
                    ...
                    <a-col :md="6" :sm="24">
                        <a-form-item label="创建人">
                            <staff-select v-model="queryParam.creator" role-name="销售员" placeholder="选择创建人" />
                        </a-form-item>
                    </a-col>
                    ...
                    <a-col :md="6" :sm="24">
                        <a-form-item label="上一售后">
                            <staff-select v-model="queryParam.lastExecutor" role-name="售后员" placeholder="选择售后员" />
                        </a-form-item>
                    </a-col>
                    <a-col :md="6" :sm="24">
                        <a-form-item label="售后员">
                            <staff-select v-model="queryParam.executor" role-name="售后员" placeholder="选择售后员" />
                        </a-form-item>
                    </a-col>
                </a-row>
            </a-form>
        ...
        onChange: onSelectChange
            ...
            <s-table size="middle" ref="table" rowKey="oid" :alert="false" :columns="columns" :data="loadData" showPagination="auto" :pagination="pagination">
                ...
                <span slot="cid" slot-scope="text,record">
                    <router-link :to="{path:'/customDetail',query:{cid:text}}">{{record.customName}}/{{text}}</router-link>
                </span>
                <span slot="paidMoney" slot-scope="text, record">
                    <router-link :to="{path:'/receiptManage',query:{orderid:record.oid}}">{{record.paidPassed | numformat}}/{{record.paidTotal | numformat}}</router-link>
                </span>
                ...
                <span slot="contract" slot-scope="text,record">
                    <router-link v-if="record.contract.way" :to="{path:'/contractManage',query:{ctid:record.contract.ctid}}">
                ...
                <span slot="action" slot-scope="text,record">
                    <div v-if="record.editing">
                        <a-button size="small" icon="check" @click="transOrder(record)" :disabled="record.executor === editingCache.executor"></a-button>
                        <a-button size="small" icon="close" @click="cancelTrans(record)"></a-button>
                    </div>
                    <div v-else>
                        <a-button type="primary" size="small" @click="transOrder1(record)" :disabled="!$auth('orderManage.alloc') || Object.keys(editingCache).length > 0">{{record.executor?'转移':'分配'}}</a-button>
                        <a-button v-if="paidOver(record)" size="small" @click="upgradeOrder(record)" :disabled="!$auth('orderManage.upgrade') || Object.keys(editingCache).length > 0">升级</a-button>
                        <a-button v-else size="small" @click="addaReceipt(record)" :disabled="!$auth('orderManage.receipt') || Object.keys(editingCache).length > 0">收/退款</a-button>
                        <a-button size="small" @click="signContract(record)" :disabled="!$auth('orderManage.contract') || Object.keys(editingCache).length > 0">合同</a-button>
                    </div>
