                ...
                <a-list itemLayout="vertical" :dataSource="appRoleList">
                    <a-list-item slot="renderItem" slot-scope="item, index" :key="index">
                        <a-list-item-meta :style="{ marginBottom: '0' }">
                            ...
                            <a slot="title" style="text-align: center; display: block" @click="edit(item)" :class="{current:currentRole === item.en}">{{ item.name }}</a>
                        </a-list-item-meta>
                    </a-list-item>
                </a-list>
            </a-col>
                    ...
                    <a-row :gutter="16" v-for="(permission, index) in permissions" :key="index">
                        <a-form-item :label="permission.name">
                            <a-col :xl="20" :lg="24">
                                ...
                                <a-checkbox-group :key="permission.name" :options="permission.actionsOptions" v-model="permission.selected" @change="onChangeCheck(permission)" />
                            </a-col>
                        </a-form-item>
                    </a-row>
                    <a-form-item>
    ...
    methods: {
        edit(record) {
            ...
            this.permissions.forEach(p => {
                ...
                p.indeterminate =
                    !!p.selected.length &&
                    p.selected.length < p.actionsOptions.length;
            });
        },
        ...
        async getRolePermissions() {
            try {
                let result = await getRolePermissions();
                console.log("getRolePermissions", result);
