<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator" style="margin-bottom:18px">
      ...
      <a-button type="primary" icon="redo" @click="resetQueryparam"
        >重置</a-button
      >
      <a-button
        :disabled="!$auth('staffManage.add')"
        type="primary"
        icon="plus"
      ...
      >
      <a-dropdown
        :disabled="!$auth('staffManage.modify')"
    ...
    >
      ...
      <span slot="status" slot-scope="text, record">
        <a-switch
          v-if="record.editing"
          :checkedChildren="workingStatus.name"
          :unCheckedChildren="offWorkingStatus.name"
          :checked="text === workingStatus.id"
          size="small"
          @change="
            checked => {
        ...
        <a-badge
          v-else
          :status="text === workingStatus.id ? 'processing' : 'error'"
          :text="
            text == workingStatus.id
              ? workingStatus.name
              : offWorkingStatus.name
          "
        />
      </span>
...
export default {
  ...
  created() {
    if (!this.$auth("staffManage.search")) {
    ...
    async stopBusiness() {
      try {
        let modifySuccess = await updateStaff({
          ...
          status: this.offWorkingStatus.id
        });
        this.$notification.success({
          $message: "成功",
