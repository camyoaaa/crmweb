<template>
  <div
    style="margin:-22px -16px 0px -15px;background:white;padding:16px 16px 0px 16px"
  >
      ...
      <a-tab-pane tab="套餐列表" key="list">
        <a-card :bordered="false">
          <div class="table-operator" style="margin-bottom:18px">
            <a-button
              type="primary"
              :disabled="!$auth('mealManage.add')"
              icon="plus"
              @click="$refs.createParent.open()"
              >新增父级套餐</a-button
            >
          </div>
          <a-card
            v-for="(pMeal, index) in mealData"
            :key="pMeal.mid"
          ...
          >
            ...
            <a-button-group slot="extra">
              <a-button
                :disabled="!$auth('mealManage.modify')"
                type="primary"
                @click="startEdit(pMeal)"
                >修改父级名字</a-button
              >
              <a-popconfirm
                :disabled="!$auth('mealManage.delete')"
                title="确定删除这个父级以及所属子级套餐吗?"
                placement="topRight"
                okText="Yes"
                cancelText="No"
                @confirm="deleteMeal(pMeal)"
                @cancel="_ => {}"
              >
                <a-button :disabled="!$auth('mealManage.delete')" type="danger"
                  >删除父级套餐</a-button
                >
              </a-popconfirm>
              <a-button
                :disabled="!$auth('mealManage.add')"
                type="primary"
                @click="$refs.createChild.open(pMeal.mid)"
                >新增子级套餐</a-button
              >
            </a-button-group>
            <a-table
              ...
              :key="pMeal.mid"
              :columns="columns"
              :dataSource="pMeal.childMealList.filter(m => m.mid)"
              :pagination="false"
            >
              ...
              <span slot="orderNumber" slot-scope="text, record">
                <router-link
                  :to="{ path: '/orderManage', query: { mid: record.mid } }"
                  >{{ text }}</router-link
                >
              </span>
              <span slot="action" slot-scope="text, record">
                ...
                <div v-else>
                  ...
                  <a-popconfirm
                    title="确定删除这个套餐吗"
                    @confirm="deleteMeal(record)"
                    @cancel="() => {}"
                    okText="Yes"
                    cancelText="No"
                    :disabled="
                      !$auth('mealManage.delete') ||
                        Object.keys(editingCache).length > 0
                    "
                  >
                    <a-button
...
export default {
  ...
  created() {
    if (!this.$auth("mealManage.search")) {
    ...
    addEditingField(array) {
      return array.map(element => {
        ...
        if (element.childMealList && element.childMealList.length > 0) {
          element.childMealList = this.addEditingField(element.childMealList);
        }
        return element;
      });
    },
