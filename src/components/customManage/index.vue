<template>
    <a-card :bordered="false">
                ...
                <a-row :gutter="48">
                    ...
                    <a-col :md="6" :sm="24">
                        <a-form-item label="接待人员">
                            <staff-select v-model="queryParam.receptionist" role-name="接待员" placeholder="请选择接待人员" />
                        </a-form-item>
                    </a-col>
                    <a-col :md="6" :sm="24">
                        <a-form-item label="上一销售">
                            <staff-select v-model="queryParam.lastSeller" role-name="销售员" placeholder="请选择销售人员" />
                        </a-form-item>
                    </a-col>
                    <a-col :md="6" :sm="24">
                        <a-form-item label="销售人员">
                            <staff-select v-model="queryParam.seller" role-name="销售员" placeholder="请选择销售人员" />
                        </a-form-item>
                    </a-col>
                    <a-col :md="6" :sm="24">
                        <a-form-item label="客户状态">
                            <custom-status-select v-model="queryParam.status" placeholder="请选择客户状态" />
                        </a-form-item>
                    </a-col>
                    <a-col :md="6" :sm="24">
                        <a-form-item label="接待时间">
                            <a-range-picker style="width:100%" v-model="queryParam.range" :showTime="{
        ...
        <div class="table-operator" style="margin-bottom:18px">
            ...
            <a-button type="primary" @click="openNewModal" icon="plus" :disabled="!$auth('customManage.add')">添加</a-button>
        </div>
        ...
        onChange: onSelectChange
            ...
            <s-table size="middle" ref="table" rowKey="cid" :alert="false" :columns="columns" :data="loadData" showPagination="auto" :pagination="pagination">
                ...
                <span slot="filterOrderList" slot-scope="text, {filterOrderList}">
                    ...
                    <div v-else>
                        <a-dropdown>
                            <div>
                                <router-link :to="{path:'/orderDetail',query:{oid:filterOrderList[0].oid}}">
                                    <ellipsis :length="10" tooltip>{{filterOrderList[0].meal.name}}</ellipsis>
                                </router-link>
                                <a v-if="filterOrderList.length > 1">+ {{filterOrderList.length - 1}}</a>
                            </div>
                            <a-menu slot="overlay" v-if="filterOrderList.length > 1">
                                <a-menu-item v-for="order in filterOrderList" :key="order.oid">
                                    <router-link :to="{path:'/orderDetail',query:{oid:order.oid}}">
                ...
                <span slot="followList" slot-scope="text, record">
                    <ellipsis :length="8" tooltip>{{getNewestRecord(record.followList)  }}</ellipsis>
                </span>
                <span slot="sellerName" slot-scope="text,record">
                    <staff-select size="small" v-if="record.editing" style="width:150px" v-model="record.seller" detail role-name='销售员' :disabled-staff="editingCache.seller" />
                    <div v-else>{{text}}</div>
                </span>
                <span slot="action" slot-scope="text,record">
                    <div v-if="record.editing">
                        <a-button size="small" icon="check" @click="transCustom(record)" :disabled="(!record.seller) || record.seller == editingCache.seller"></a-button>
                        <a-button size="small" icon="close" @click="cancelTrans(record)"></a-button>
                    </div>
                    <div v-else>
                        <a-button type="primary" size="small" @click="followCustom(record.cid)" :disabled="!$auth('customManage.follow') || Object.keys(editingCache).length > 0">跟进</a-button>
                        <a-button size="small" @click="openOrderModal(record)" :disabled="!$auth('customManage.order') ||Object.keys(editingCache).length > 0">签单</a-button>
                        <a-button size="small" @click="transCustom1(record)" :disabled="!$auth('customManage.alloc') ||Object.keys(editingCache).length > 0">转移</a-button>
                    </div>
