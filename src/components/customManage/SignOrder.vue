  ...
  >
    <a-alert type="warning" v-if="!hasDetailData" showIcon>
      ...
      <div slot="description">
        签单前请确认已经补充好客户的完整信息
        <router-link
          :to="{ path: '/customFollow', query: { cid: customInfo.cid } }"
          >去补充</router-link
        >
      </div>
    </a-alert>
      ...
      <a-spin :spinning="spinning">
        <a-form-model ref="orderForm" :model="order" :rules="orderRules">
          <a-row :gutter="48">
            ...
            <a-col :span="12">
              <a-form-model-item label="售后员" prop="executor">
                <staff-select
...
export default {
  ...
  data() {
    return {
      ...
      receipt: {}, //收据信息
      orderRules: {
        mid: [{ required: true, message: "请选择套餐", trigger: "change" }],
        money: [
          {
            required: true,
            message: "请输入订单金额",
