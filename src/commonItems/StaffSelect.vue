<template>
    <a-select :size="size" :mode="mode" :value="value" @change="onSelect" optionLabelProp="label" style="width:100%" :placeholder="placeholder" allowClear>
        <a-select-option v-for="staff in staffList" :key="staff.account" :value="staff.account" :label="staff.name" :disabled="
        (detail && staff.status === offworkStatus) ||
          disabledStaff === staff.account
      ">
            ...
            try {
                ...
                if (
                    ...
                    if (this.preselect && !this.value) {
                        let theFirstWorkingStaff = this.staffList.find(
                            s => s.status !== this.offworkStatus
                        ); //选中第一个工作中的员工
                        // this.$emit("change", this.staffList[0].account);
                        this.$emit("change", theFirstWorkingStaff.account);
                    }
                }
            } catch (error) {
                console.log(error);
